...
> **9-12 阶段小计**：11 分类，**79 项目**，已完成 **79**（A·函数 8/8 + B·变量 8/8 + C·多角色 8/8 + D·键盘 8/8 + E·音乐 8/8 + F·数学 8/8 + G·列表 8/8 + H·综合小游戏 8/8 + I·故事 6/6 + J·科学 7/7 + **读代码 code 2/2**；全部上线，无待建）。全部轻量运行时（自定义积木/变量/多角色/键盘/音效/舞台点击/列表 7 原语/时间轴引擎）已落地。
> **读代码分类（2026-08-30 新增）**：9-12 的最后一级结构化台阶，位于 J·科学之后、自由创作之前。
> 形态是「只读代码 + 预测结果」的选择题（`component: "codequiz"`，题目在项目数据的 `quiz` 字段，
> 不产生积木 XML、也不要求动手写代码），承接 9-12 学习页刚开放的「生成的 JavaScript」预览，
> 为 13-16 手写 JS 补齐阅读能力这一环。两个项目：`read_loops`（循环/转角/变量累加）、
> `read_vars`（变量设定与累加、循环总数、if 判断）。
...
| 9-12 岁 | 11 | 79 | **79** | 全部轻量运行时已落地（含列表 7 原语 / 时间轴引擎）；9-12 阶段 11 分类全部满编（含新增「读代码」2 项） |
...
**当前下一步**：6-8（111/111）、9-12（79/79）、13-16（53/53）三阶段已全部满编，全站累计 **243 个**引导项目（含造物工坊）均已上线。
...
> 约定：所有新增项目默认接入对应阶段的 `projectSlugs`（stage-6-8 已含 111 项；stage-9-12 已含 79 项；stage-13-16 已含 10 项）。每次完善后同步更新本表，保证「计划」与「实况」一致。
> ⚠️ **新增项目后必跑 `tests/path.test.ts`**：该测试硬编码了 6-8 阶段 `linearOrder.length`（现为 111）。
> 2026-08-30 曾因 PBL 补到 10 项后没同步它，造成 2 个断言失败却一度被误判为「沙箱网络非回归」——
> 教训：全量回归里出现 failed 时，必须先单独重跑可疑文件确认是 `AssertionError`（真回归）
> 还是环境报错（非回归），不能凭文件类型想当然。
>
