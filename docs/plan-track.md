...
> 现状：已上线 **182 个**项目（6-8 阶段 **105 个**·11 分类全交付 + 9-12 阶段 **77 个**：A·函数/B·变量/C·多角色/D·键盘/E·音乐/F·数学/G·列表/H·综合小游戏 各 8/8、I·故事 6/6、J·科学 7/7，均已上线；13-16 阶段 启动（js 分类试点 `js_square` 已上线，JS 模式地基已建））。
...
> `LearnPageClient` / `DemoOverlay` 已按 `codeMode` 切分支。剩余「画布与 DOM 渲染」类项目（分类 M/N/O/P）留待后续 Phase。
...
> 实现分类 id = `js`。Phase 0 已交付首个试点（✅ `js_square`），验证「写 JS → 判完成 → 看示范」最小闭环。
- ✅ [JS模式] JS 循环画图形（`js_square` · 落笔 + for 循环画正方形，已上线）
- ⬜ [需: JS模式] 第一个 JS 程序：打印
- ⬜ [需: JS模式] JS 变量与运算
- ⬜ [需: JS模式] JS 函数
- ⬜ [需: JS模式] JS 数组
...
> **13-16 阶段小计**：8 分类，约 60 项目，已完成 **1**（js 分类试点 `js_square`；JS 模式地基 Phase 0 已建，其余分类按 Phase 节奏铺开）。
...
| 13-16 岁 | 8 | 53 | 1 | 文本 JS 模式（地基已建）/ 画布 / DOM / 算法与数据 |
