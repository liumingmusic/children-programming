...
> 现状：已上线 **181 个**项目（6-8 阶段 **105 个**·11 分类全交付 + 9-12 阶段 **76 个**：A·函数/B·变量/C·多角色/D·键盘/E·音乐/G·列表/H·综合小游戏 各 8/8、F·数学 7/8（数独待补）、I·故事 6/6、J·科学 7/7，均已上线；仅 F·数独 1 项待建；13-16 阶段 待建）。
...
>
> 现状（2026-08-26）：分类 A·函数 8/8、B·变量 8/8、C·多角色 8/8、D·键盘 8/8、E·音乐 8/8、F·数学 7/8（数独待补「列表」）、G·列表 8/8、H·综合小游戏 8/8、I·故事 6/6、J·科学 7/7，均已上线（内容+判定+测试就绪，本轮 G/J/H 三批随 gh-pages 部署同步上线）。所需轻量运行时——自定义积木 / 变量·计分 / 多角色碰撞·距离·消息广播 / 键盘(含 apples 下落物) / 音效 / 舞台点击 / **列表 7 原语** / 时间轴引擎——**全部到位**。剩余缺口：仅 F·数独 1 项待建（「列表」基石已由 G 落地，下一批次可补）。
...
- 🔧 [需: 列表] 数独填空 `math_sudoku`（「列表」基石已由 G 分类落地，下一批次可补；运行时尚需确认 9×9 校验逻辑）
...
> **9-12 阶段小计**：10 分类，**77 项目**，已完成 **76**（A·函数 8/8 + B·变量 8/8 + C·多角色 8/8 + D·键盘 8/8 + E·音乐 8/8 + F·数学 7/8 + G·列表 8/8 + H·综合小游戏 8/8 + I·故事 6/6 + J·科学 7/7；仅 F·数独 1 项待建）。全部轻量运行时（自定义积木/变量/多角色/键盘/音效/舞台点击/列表 7 原语/时间轴引擎）已落地。
...
| 9-12 岁 | 10 | 77 | 76（仅 F·数独 1 待建） | 全部轻量运行时已落地（含列表 7 原语 / 时间轴引擎）；仅 F·数独 待建 |
